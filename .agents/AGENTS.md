...
## Reactive Wakeup Rule (CRITICAL)

- Do NOT constantly poll logs or run terminal commands (like tail) while a long background task is running.
- Stop calling tools immediately (go "OFF" / IDLE) and let the system wait automatically for the task to finish via Reactive Wakeup.
- The user hates it when the agent keeps doing things and wasting time while waiting for a process. Always respect this.
